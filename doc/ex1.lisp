...
(progn
  (with-open-file (s "emitted_code.cpp" :direction :output
                     :if-exists :supersede
                     :if-does-not-exist :create)
    (emit-cpp :str s :clear-env t
              :code 
              `(with-compilation-unit
                   (include "GLFW/glfw3.h")
                 (function (main ((argc :type int) (argv :type char**))
                                 int)
                           (decl ((argc :type (void))))
                           (macroexpand
                            (with-glfw-window (main_window :w 512 :h 512)
                              ...
                              #+black-on-white
                              (statements (funcall glClearColor 1.0 1.0 1.0 1.0)
  ...
  (sb-ext:run-program "/usr/bin/clang-format"
                      (list "-i" (namestring "emitted_code.cpp"))))
